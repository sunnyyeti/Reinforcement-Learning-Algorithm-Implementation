            ...
            for req2 in range(CarCompany.POISSON_UPBOUND + 1):
                ...
                for ret1 in range(CarCompany.POISSON_UPBOUND + 1):
                    ...
                    for ret2 in range(CarCompany.POISSON_UPBOUND + 1):
                        ...
                        first_loc = min(first_loc + ret1, CarCompany.MAX_CARS)
                        second_loc = min(second_loc + ret2, CarCompany.MAX_CARS)
                        prob = p1 * p2 * p3 * p4
                        reward += prob * (benefits + CarCompany.DISCOUNT * self.state_values[first_loc, second_loc])
        return reward

    def policy_evaluation(self, epsilon=1e-5):
        step = 0
        while True:
            step+=1
            print(step)
...
def figure_4_2():
    ...
    while True:
        ...
        fig = seaborn.heatmap(np.flipud(carcomp.policy))
        ...
        if policy_stable:
            ...
            x,y = np.meshgrid(x,y)
            ax.plot_surface(x,y,CarCompany.state_values,cmap=cm.coolwarm)
