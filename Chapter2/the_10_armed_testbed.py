...
class Bandit():
    ...
    def __init__(self, k_arms, q_true_mean, q_true_std, arm_reward_std, q_estimation_initial, strategy, sample_average, step_size):
        """
    ...
    def take_action(self):
        if self.strategy["name"] == Bandit.GREEDY:
            max_est = np.max(self.q_estimation)
        ...
        if self.strategy["name"] == Bandit.EPS_GREEDY:
            epsilon = self.strategy.pop["epsilon"]
