...
class Blackjack:
    ...
    def state_value_estimation(self, episodes):
        ...
        state_cnts = {True: np.zeros((10, 10), dtype=np.float64) + 1e-12,
                      False: np.zeros((10, 10), dtype=np.float64) + 1e-12}
        for _ in tqdm(range(episodes)):
            ...
            for player_sum, dealer_show, player_useace, _ in player_trajectory:
                state_values[player_useace][player_sum - 12][dealer_show - 1] += reward
                state_cnts[player_useace][player_sum - 12][dealer_show - 1] += 1.0
    ...
    def off_policy_fixed_state_value_estimation(self, episodes, method):
        behaviour_poliy = lambda _: np.random.choice([Participant.HIT, Participant.STICK])
        dealer_policy = np.ones(Participant.MAX + 1, dtype=np.int8)
        for i in range(17, Participant.MAX + 1):
            dealer_policy[i] = Participant.STICK
        dealer_policy = Dealer_Policy(dealer_policy)
        self.dealer.set_policy(dealer_policy)
        target_policy = np.ones((10, 10, 2), dtype=np.int8)  ##sum(12-21), dealer_show(1-10), usable_ace
        target_policy[-2:, :, :] = Participant.STICK
        target_policy = Player_Policy(target_policy)
        self.player.set_policy(behaviour_poliy)
        PLAYER_SUM = 13
        PLAYER_USABLE_ACE = True
        DEALER_SHOW = 2
        state_values = np.zeros(episodes,dtype=np.float32)
        state_accured_scaled_reward = 0.0
        state_accured_weights = 1e-12
        for episode in range(episodes):
            self.player.init(PLAYER_SUM,PLAYER_USABLE_ACE,None)
            self.dealer.init(DEALER_SHOW)
            player_trajectory = self.player.run()
            if self.player.burst:
                reward = -1
            ...
            scale = 1.0
            for player_sum, dealer_card, player_useace, act in player_trajectory:
                if act==target_policy((player_sum,dealer_card,player_useace)):
                    scale*=2.0
                else:
                    scale=0.0
                    break
            state_accured_scaled_reward+=scale*reward
            if method=="ordinary":
                state_accured_weights+=1.0
            else:
                state_accured_weights+=scale
            state_values[episode] = state_accured_scaled_reward/state_accured_weights
        return state_values
...
def figure_5_2():
    ...
    draw_policy_heatmap(1, np.argmax(usable_ace_action_values, axis=-1), "Dealer showing", "Player sum",
                        "Usable ace; Policy")
    draw_policy_heatmap(3, np.argmax(no_usable_ace_action_values, axis=-1), "Dealer showing", "Player sum",
                        "No usable ace; Policy")
    draw_state_value(2, usable_ace_action_values.max(axis=-1), "Dealer showing", "Player sum",
                     "Usable ace; State value")
    draw_state_value(4, no_usable_ace_action_values.max(axis=-1), "Dealer showing", "Player sum",
                     "No usable ace; State value")
...
def figure_5_3():
    ...
    runs = 100
    episodes = 10000
    state_values = np.zeros((2,runs,episodes),dtype=np.float32)
    methods = ["ordinary","weighted"]
    baseline = -0.27726
    for m,method in enumerate(methods):
        for run in tqdm(range(runs)):
            state_vs = blackjack.off_policy_fixed_state_value_estimation(episodes,method)
            state_values[m,run,:] = state_vs
    ordinary = np.power(state_values[0]-baseline,2).mean(axis=0)
    weighted = np.power(state_values[1]-baseline,2).mean(axis=0)
    plt.figure()
    plt.plot(np.arange(1,episodes+1),ordinary,label="Ordinary importance sampling")
    plt.plot(np.arange(1,episodes+1),weighted,label="Weighted importance sampling")
    plt.xscale("log")
    plt.ylabel("Mean squared error (average over {} runs)".format(runs))
